        ...
        if self.batch is not None:
            if interleave:
                ...
                if isinstance(repeat_times, list):
                    repeats_tensor = torch.tensor(repeat_times)
                    new_batch_size = sum(repeat_times)
                else:
                    repeats_tensor = repeat_times
                    new_batch_size = self.batch.batch_size[0] * repeat_times

                repeated_tensors = {
                    key: tensor.repeat_interleave(repeats_tensor, dim=0) for key, tensor in self.batch.items()
                }
            else:
                # Stack the data
                if not isinstance(repeat_times, int):
                    raise ValueError("A list of repeat_times is not supported when interleave=False.")
            
                repeats_tensor = repeat_times
                new_batch_size = self.batch.batch_size[0] * repeat_times

                repeated_tensors = {
                    key: tensor.unsqueeze(0).expand(repeats_tensor, *tensor.shape).reshape(-1, *tensor.shape[1:])
