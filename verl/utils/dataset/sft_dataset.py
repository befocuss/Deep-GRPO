...
class SFTDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        if "prefix" in self.dataframe.columns:
            self.prefixes = self.dataframe["prefix"].tolist()
        else:
            self.prefixes = [None] * len(self.prompts)
    ...
    def __getitem__(self, item):
        ...
        prefix_str = self.prefixes[item]
        if prefix_str is not None and len(prefix_str) > 0:
            prefix_ids_out = tokenizer(prefix_str, return_tensors="pt", add_special_tokens=False)
            prefix_ids = prefix_ids_out["input_ids"][0]

            match_len = 0
            min_len = min(len(prefix_ids), len(response_ids))

            for i in range(min_len):
                if prefix_ids[i] == response_ids[i]:
                    match_len += 1
                else:
                    break

            start_idx = max(0, prompt_length - 1) 
            end_idx = min(start_idx + match_len, loss_mask.size(0))

            loss_mask[start_idx : end_idx] = 0
