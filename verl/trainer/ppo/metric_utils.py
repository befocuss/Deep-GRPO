...
def compute_data_metrics(batch: DataProto, max_model_len: int, prefix: str) -> Dict[str, Any]:
    """
    ...
    is_clipped = torch.ge(total_length, max_model_len)

    sequence_reward = batch.batch["token_level_rewards"].sum(-1)    

    metrics = {
        # # reward
        f"critic/{prefix}/rewards/mean": torch.mean(sequence_reward).detach().item(),
        f"critic/{prefix}/rewards/max": torch.max(sequence_reward).detach().item(),
        f"critic/{prefix}/rewards/min": torch.min(sequence_reward).detach().item(),
        # adv
        f"critic/{prefix}/advantages/mean": torch.mean(valid_adv).detach().item(),
        f"critic/{prefix}/advantages/max": torch.max(valid_adv).detach().item(),
        f"critic/{prefix}/advantages/min": torch.min(valid_adv).detach().item(),
        # response length
        ...
        # sequence length
        f"sequence_length/{prefix}/mean": torch.mean(total_length).detach().item(),
        f"sequence_length/{prefix}/max": torch.max(total_length).detach().item(),
        f"sequence_length/{prefix}/min": torch.min(total_length).detach().item(),
        f"sequence_length/{prefix}/clip_ratio": torch.mean(is_clipped.float()).detach().item(), 
    }

    # multi-turn conversation
    if "__num_turns__" in batch.non_tensor_batch:
        num_turns = batch.non_tensor_batch["__num_turns__"]
        metrics[f"num_turns/{prefix}/min"] = num_turns.min()
        metrics[f"num_turns/{prefix}/max"] = num_turns.max()
        metrics[f"num_turns/{prefix}/mean"] = num_turns.mean()

    return metrics


def compute_timing_metrics(timing_raw: Dict[str, float]) -> Dict[str, Any]:
    """
...
def compute_throughout_metrics(batch: DataProto, timing_raw: Dict[str, float], n_gpus: int) -> Dict[str, Any]:
    ...
    """
    total_num_tokens = sum(batch.non_tensor_batch["global_token_num"])
