...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        # See https://github.com/anonymous/anonymous-repo/blob/master/examples/ray/tutorial.ipynb for more information.
        all_wg = {}
    ...
    def _process_hard_failure_seeds(self, hard_failure_seeds):
        results = []
        
        for item in hard_failure_seeds:
            prompt_ids = item['prompt_ids']
            response_ids = item['response_ids']
            raw_prompt_ids = item['data_instance']['raw_prompt_ids']

            prompt_text = item["data_instance"]["raw_prompt"][0]["content"]
            
            len_raw = len(raw_prompt_ids)
            assert prompt_ids[:len_raw] == raw_prompt_ids
            
            full_response_ids = prompt_ids[len_raw:] + response_ids
            
            response_text = self.tokenizer.decode(full_response_ids, skip_special_tokens=True)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        main_chain_batch, branch_chain_batch, hard_failure_seeds = self.async_rollout_manager.ts_generate_sequences(gen_batch)
                        if len(branch_chain_batch) < 128: # TODO: we ensure minimum nuber of branch chain examples for computing gradient, make it configurable later
                            branch_chain_batch = DataProto(batch=None, non_tensor_batch={}, meta_info={})

                    if hard_failure_seeds and len(hard_failure_seeds) > 0:
                        hard_failure_seeds = self._process_hard_failure_seeds(hard_failure_seeds)
                        seed_save_path = os.path.join(self.config.trainer.default_local_dir, "hard_failure_seeds.jsonl")

                        os.makedirs(os.path.dirname(seed_save_path), exist_ok=True)
                        
                        with open(seed_save_path, 'a', encoding='utf-8') as f:
                            for seed in hard_failure_seeds:
                                seed_data = seed
                                seed_data['global_step'] = self.global_steps
                                seed_data['epoch'] = epoch
                                
                                f.write(json.dumps(seed_data, ensure_ascii=False) + '\n')
                                
                        metrics.update({"data/hard_failure_seeds_count": len(hard_failure_seeds)})
                    ...
                    main_chain_batch.non_tensor_batch["sources"] = np.zeros(len(main_chain_batch))
                    branch_chain_batch.non_tensor_batch["sources"] = np.ones(len(branch_chain_batch))

                    # whiten advantages
                    if self.config.trainer.whiten_advantages:
                        main_chain_advs = main_chain_batch.batch["advantages"]
                        main_chain_response_mask = main_chain_batch.batch["response_mask"].bool()
                        valid_main_chain_advs = torch.masked_select(main_chain_advs, main_chain_response_mask)
                        if valid_main_chain_advs.numel() > 1:
                            valid_main_chain_adv_mean = valid_main_chain_advs.mean()
                            valid_main_chain_adv_std = valid_main_chain_advs.std()
                            main_chain_batch.batch["advantages"] = (main_chain_advs - valid_main_chain_adv_mean) / (valid_main_chain_adv_std + 1e-8)
                            main_chain_batch.batch["advantages"] = main_chain_batch.batch["advantages"] * main_chain_response_mask
                        
                        if len(branch_chain_batch) > 0:
                            branch_chain_advs = branch_chain_batch.batch["advantages"]
                            branch_chain_response_mask = branch_chain_batch.batch["response_mask"].bool()   
                            valid_branch_chain_advs = torch.masked_select(branch_chain_advs, branch_chain_response_mask)
                            if valid_branch_chain_advs.numel() > 1:
                                valid_branch_chain_adv_mean = valid_branch_chain_advs.mean()
                                valid_branch_chain_adv_std = valid_branch_chain_advs.std()
                                branch_chain_batch.batch["advantages"] = (branch_chain_advs - valid_branch_chain_adv_mean) / (valid_branch_chain_adv_std + 1e-8)
                                branch_chain_batch.batch["advantages"] = branch_chain_batch.batch["advantages"] * branch_chain_response_mask

                    # merge main_chain_batch and branch_chain_batch and remove the tail
                    batch = main_chain_batch
                    if len(branch_chain_batch) > 0:
                        batch = DataProto.concat([batch, branch_chain_batch])
                    batch = batch[:len(batch) - (len(batch) % self.actor_rollout_wg.world_size)]
                    ...
                    # add main_chain_mask and branch_chain_mask
                    ...
                    assert T_main + T_branch == T_total
                    ...
                    with marked_timer("update_actor", timing_raw, color="red"):
                        ...
                        actor_output_metrics = reduce_metrics(actor_output.non_tensor_batch["metrics"])
                    ...
                    if rollout_data_dir:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            reward_infos = batch.non_tensor_batch["__reward_infos__"]
                            reward_info_keys = reward_infos[0].keys()
                            reward_extra_infos_dict = {k: [d[k] for d in reward_infos] for k in reward_info_keys if k != 'reward'}
                            self._dump_generations(
                                inputs=inputs,
                                outputs=outputs,
                                scores=scores,
