...
def agg_loss(loss_mat: torch.Tensor, 
    ...
    Args:
        ...
        weights: `(torch.Tensor)` (Optional):
            shape: (bs, response_length) or (bs, 1) broadcastable.
            If provided, the function calculates sum(loss_mat * loss_mask * weights).
            This is designed for Global-Token-Num normalized loss (Mixed Batch FSDP).
    Returns:
        loss: `a scalar torch.Tensor` (or tensor if mode is 'none')
            aggregated loss
    """

    if weights is not None:
        loss = torch.sum(loss_mat * loss_mask * weights)
        return loss
...
def compute_policy_loss(
    ...
    pg_clips = torch.gt(pg_losses2, pg_losses1).float()

    pg_losses3 = -advantages * clip_ratio_c
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lowers = torch.gt(clip_pg_losses1, pg_losses3) * (advantages < 0).float()

    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)

    return pg_losses, pg_clips, ppo_kl, pg_clipfrac_lowers
