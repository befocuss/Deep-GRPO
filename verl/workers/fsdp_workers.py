...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            global_num_tokens = data.non_tensor_batch["global_token_num"]
            ...
            sp_rank = 0
            if self.ulysses_sharding_manager.device_mesh is not None:
                sp_rank = self.ulysses_sharding_manager.device_mesh["sp"].get_local_rank()
            if sp_rank == 0:
                metrics = np.array([metrics], dtype=object)
            else:
                metrics = np.array([], dtype=object)
            output = DataProto(non_tensor_batch={"metrics": metrics})
