...
OUTPUT_DIR=/data
RUN_NAME=Qwen2.5-Math-1.5B-MATH-CRGRPO
...
/workspace/miniconda3/bin/conda run -p /workspace/miniconda3/envs/verl --no-capture-output python3 -m  verl.trainer.main_ppo \
    ...
    data.train_files="[$DATA_DIR/math-oat/train.parquet]" \
    data.val_files="[$DATA_DIR/math-oat/test.parquet]" \
    ...
    actor_rollout_ref.rollout.spo.branches_per_point=4 \
    actor_rollout_ref.rollout.spo.pick_branch_chain_root_method=random \
