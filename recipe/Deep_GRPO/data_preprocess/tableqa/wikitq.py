...
def create_prompt_from_wikitq(
    ...
    if is_think:
        from recipe.spo.data_preprocess.tableqa.prompt_think import COT_PROMPT_TEMPLATE

        prompt = COT_PROMPT_TEMPLATE.format(table=table_str, question=item["question"])
    else:
        from recipe.spo.data_preprocess.tableqa.prompt import COT_PROMPT_TEMPLATE
